...
namespace Application.Agents.AzureDevOps
{
    ...
    public class UnitTestAgentRunner : BaseAgentRunner
    {
        ...
        protected override async Task<AgentResult> ExecuteAsync(AgentRequest request, CancellationToken ct)
        {
            ...
            // Límite de seguridad — máximo 15 llamadas a herramientas
            // Si el agente no terminó en 15 pasos, algo está en loop
            const int maxToolCalls = 15;

            // Hook para contar y frenar si se pasa del límite
            kernel.FunctionInvocationFilters.Add(new ToolCallLimitFilter(maxToolCalls, _loggerFactory.CreateLogger<UnitTestAgentRunner>()));
